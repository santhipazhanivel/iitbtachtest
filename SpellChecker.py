...
def inference_decoding_layer(embeddings, start_token, end_token, dec_cell, initial_state, output_layer,
    ...
    with tf.name_scope("Inference_Decoder"):
        ...
        inference_helper = tf.tf.compat.v1.estimator.seq2seq.GreedyEmbeddingHelper(embeddings,
                                                                    start_tokens,
                                                                    end_token)

        inference_decoder = tf.tf.compat.v1.estimator.seq2seq.BasicDecoder(dec_cell,
                                                            inference_helper,
                                                            initial_state,
                                                            output_layer)

        inference_logits, _ = tf.tf.compat.v1.estimator.seq2seq.dynamic_decode(inference_decoder,
                                                                output_time_major=False,
                                                                impute_finished=True,
                                                                maximum_iterations=max_target_length)
...
def decoding_layer(dec_embed_input, embeddings, enc_output, enc_state, vocab_size, inputs_length, targets_length, 
    ...
    with tf.name_scope("RNN_Decoder_Cell"):
        for layer in range(num_layers):
            with tf.variable_scope('decoder_{}'.format(layer)):
                lstm = tf.tf.compat.v1.estimator.rnn.LSTMCell(rnn_size)
                dec_cell = tf.tf.compat.v1.estimator.rnn.DropoutWrapper(lstm, 
                                                         input_keep_prob = keep_prob)
    
    output_layer = Dense(vocab_size,
                         kernel_initializer = tf.truncated_normal_initializer(mean = 0.0, stddev=0.1))
    
    attn_mech = tf.tf.compat.v1.estimator.seq2seq.BahdanauAttention(rnn_size,
                                                  enc_output,
                                                  inputs_length,
                                                  normalize=False,
    ...
    with tf.name_scope("Attention_Wrapper"):
        dec_cell = tf.tf.compat.v1.estimator.seq2seq.DynamicAttentionWrapper(dec_cell,
                                                              attn_mech,
                                                              rnn_size)
    
    initial_state = tf.tf.compat.v1.estimator.seq2seq.DynamicAttentionWrapperState(enc_state,
...
def build_graph(keep_prob, rnn_size, num_layers, batch_size, learning_rate, embedding_size, direction):
    ...
    with tf.name_scope("cost"):
        # Loss function
        cost = tf.tf.compat.v1.estimator.seq2seq.sequence_loss(training_logits, 
                                                targets, 
                                                masks)
